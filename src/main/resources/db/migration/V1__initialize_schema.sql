-- Baseline schema creation for new environments.
-- Uses IF NOT EXISTS guards so it is safe to run on partially initialized databases.
...
CREATE TABLE IF NOT EXISTS daily_insights (
    ...
    CONSTRAINT fk_daily_insights_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
-- refresh_tokens table: create only if it doesn't exist
-- Note: Existing production tables may have different columns, so we'll add missing columns in V2
...
-- Only create indexes on columns that definitely exist in the base table
